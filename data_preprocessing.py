...
def split_train_validation(
    ...
    train_ratio : float, optional
        ...
        If None, uses config.TRAIN_RATIO
    random_seed : int, optional
        Random seed for reproducibility
        If None, uses config.SKLEARN_SEED
    stratify : bool, optional
        If True, maintain class proportions in both splits
        If None, uses config.STRATIFY_SPLIT
    ...
    Split: 150000 train, 50000 validation (75.0%/25.0%)
    """
...
def prepare_data(
    ...
    datasets : dict
        ...
        As returned by load_all_datasets()
    train_snr : int, optional
        SNR value to use for training
        If None, uses config.TRAIN_SNR
    train_ratio : float, optional
    ...
    Example:
    --------
    >>> X_train, y_train, X_val, y_val, X_test_dict, y_test_dict = prepare_data(datasets)
    ======================================================================
...
def normalize_features(
    ...
    method : str, optional
        ...
        If None, uses config.NORMALIZATION_TYPE
    verbose : bool, default=True
    ...
    ...     X_train, X_val, X_test_dict, method="standard"
    ... )
    Normalization method: StandardScaler
    Fitting scaler on training data (150000 samples)...
    Training data - Mean: 0.000000, Std: 1.000000
    """

    # Set default method
    if method is None:
        method = config.NORMALIZATION_TYPE
...
def save_scaler(scaler: Any, filepath: Optional[str] = None, verbose: bool = True) -> str:
    """
    ...
    filepath : str, optional
        ...
        If None, uses config.MODELS_DIR + config.SCALER_FILENAME
    verbose : bool, default=True
    ...
    Example:
    --------
    >>> path = save_scaler(scaler)
    Scaler saved to: ./models/feature_scaler.pkl
    """

    # Set default filepath
    if filepath is None:
        filepath = os.path.join(config.MODELS_DIR, config.SCALER_FILENAME)
...
def load_scaler(filepath: Optional[str] = None, verbose: bool = True) -> Any:
    """
...
def preprocess_pipeline(
    ...
    config.NORMALIZE_FEATURES is set to False to match the paper.
    ...
    normalize : bool, optional
        ...
        If None, uses config.NORMALIZE_FEATURES
...
def get_data_statistics(
    ...
    stats : dict
        ...
        - train_class_dist: Training class distribution
        - val_class_dist: Validation class distribution
        - feature_stats: Feature statistics (min, max, mean, std)
    """
...
def verify_class_balance(y_train: np.ndarray, y_val: np.ndarray, tolerance: float = 0.1) -> bool:
    """
...
if __name__ == "__main__":
    """
    Self-test for data_preprocessing module.
    """

    print("\n" + "=" * 70)
    print("DATA PREPROCESSING MODULE - SELF TEST")
    print("=" * 70)
